...
def check_learn_condition():
    try:
        url = f"https://firestore.googleapis.com/v1/projects/{FIREBASE_PROJECT_ID}/databases/(default)/documents/vibration/data?key={FIREBASE_API_KEY}"
...
def get_learn_phase_info():
    try:
        ...
        return f"{vibration_name}_{phase_name}"
    except:
...
def run_training(custom_config=None):
    ...
    viz_dir = os.path.join(temp_dir, 'vibration_visualizations')
    try:
        ...
        tflite_path = os.path.join(temp_dir, 'vibration_anomaly_detector.tflite')
        params_path = os.path.join(temp_dir, 'vibration_params.json')
        detector.convert_to_tflite(tflite_path)
        detector.save_params(params_path)
        
        status_info["message"] = "Uploading to Supabase..."
        upload_to_supabase(tflite_path, 'vibration_anomaly_detector.tflite')
        upload_to_supabase(params_path, 'vibration_params.json')
...
with gr.Blocks(title="Vibration Model Trainer") as demo:
    gr.Markdown("# 📳 Vibration Anomaly Model Trainer")
    with gr.Tab("📊 Status"):
    ...
    with gr.Tab("⚙️ Manual Training"):
        ...
        with gr.Row():
            ...
            bottleneck_input = gr.Slider(1, 4, value=2, step=1, label="Bottleneck Dim")
        with gr.Row():
            hidden_input = gr.Slider(3, 8, value=4, step=1, label="Hidden Dim")
