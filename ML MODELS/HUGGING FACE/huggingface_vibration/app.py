...
class VibrationAnomalyDetector:
    ...
    def load_data_from_firebase(self, data_path):
        ...
        try:
            ...
            for key, entry in data.items():
                try:
                    feature_vector = [
                        float(entry['vibMagnitude']),
                        float(entry['vibRMS_X']),
                        float(entry['vibRMS_Y']),
                        float(entry['vibRMS_Z']),
                        float(entry['vibStdDev'])
                    ]
    ...
    def generate_visualizations(self, history, output_dir):
        ...
        layers_info = [
            ('Input\n(5 features)', 1, '#F3E5F5'),
            (f'Dense\n({self.config["hidden_dim"]} units)', 3, '#E1BEE7'),
            (f'Dropout\n({self.config["dropout_rate"]})', 4, '#CE93D8'),
            (f'Bottleneck\n({self.config["bottleneck_dim"]} units)', 5, '#BA68C8'),
            (f'Dense\n({self.config["hidden_dim"]} units)', 6, '#E1BEE7'),
            (f'Dropout\n({self.config["dropout_rate"]})', 7, '#CE93D8'),
            ('Output\n(5 features)', 9, '#F3E5F5')
        ]
...
"""
        axes[1].text(0.1, 0.5, metrics_text, fontsize=11, family='monospace', verticalalignment='center',
                    transform=axes[1].transAxes, bbox=dict(boxstyle='round', facecolor='#f3e5f5', edgecolor='gray'))
    ...
    try:
        ...
        if custom_phase and custom_phase.strip():
            try:
                ...
                fields = response.json().get('fields', {})
                vibration_name = fields.get('vibration_name', {}).get('stringValue', 'vibration_data')
                data_path = f"{vibration_name}_{custom_phase.strip()}"
            except:
                data_path = f"vibration_data_{custom_phase.strip()}"
        else:
...
def get_last_training_info_from_supabase():
    ...
    try:
        url = f"{SUPABASE_URL}/storage/v1/object/public/{SUPABASE_BUCKET}/vibration_params.json"
        response = requests.get(url, timeout=10)
        if response.status_code == 200:
...
def get_status_html():
    ...
    <div style="padding:15px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:15px;">
            <div style="background:#f3e5f5;padding:10px;border-radius:5px;text-align:center;border:1px solid #9C27B0;">
                <div style="font-size:12px;color:#7b1fa2;font-weight:bold;">Last Trained</div>
                <div style="font-size:14px;font-weight:bold;color:#4a148c;">{last_trained or "Never"}</div>
            </div>
            <div style="background:#ede7f6;padding:10px;border-radius:5px;text-align:center;border:1px solid #673AB7;">
                <div style="font-size:12px;color:#512da8;font-weight:bold;">Data Path</div>
                <div style="font-size:14px;font-weight:bold;color:#311b92;">{data_path}</div>
            </div>
        </div>
    </div>
    """
...
# Build the interface
with gr.Blocks(title="Vibration Anomaly Model Trainer", theme=gr.themes.Soft()) as demo:
    gr.Markdown("# Vibration Anomaly Detection Model Trainer")
    gr.Markdown("Autoencoder-based anomaly detection for vibration sensor data")
