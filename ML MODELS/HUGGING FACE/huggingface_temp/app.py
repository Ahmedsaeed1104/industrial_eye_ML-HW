...
status_info = {"message": "Waiting for learn=true in Firestore...", "is_training": False, "countdown": 0, "last_trained": None, "last_check": None, "metrics": None, "config": DEFAULT_CONFIG.copy()}
...
class TemperatureAnomalyDetector:
    ...
    def load_data_from_firebase(self, data_path):
        ...
        try:
            ...
            for key, entry in data.items():
                try:
                    feature_vector = [float(entry['tempCurrent']), float(entry['tempDerivative']), float(entry['tempMovingAverage']), float(entry['tempStdDev'])]
    ...
    def train(self, normal_data):
        ...
        if self.config['use_early_stopping']:
            callbacks.append(keras.callbacks.EarlyStopping(monitor='val_loss', patience=self.config['early_stopping_patience'], restore_best_weights=True))
    ...
    def _calculate_metrics(self, data, reconstructions, mse, history):
        ...
        self.training_metrics = {'reconstruction_accuracy': round(reconstruction_accuracy, 2), 'final_training_loss': round(history.history['loss'][-1], 6), 'final_validation_loss': round(history.history['val_loss'][-1], 6), 'best_validation_loss': round(min(history.history['val_loss']), 6), 'normal_detection_rate': round(normal_detection_rate, 2), 'threshold': round(self.threshold, 6), 'total_samples': len(data), 'epochs_trained': len(history.history['loss']), 'mean_reconstruction_error': round(reconstruction_error, 6)}
    ...
    def generate_visualizations(self, history, output_dir):
        ...
        reconstructions = self.model.predict(self.processed_data, verbose=0)
        mse = np.mean(np.power(self.processed_data - reconstructions, 2), axis=1)
        feature_names = ['TempCurrent', 'TempDerivative', 'TempMovingAvg', 'TempStdDev']
        
        fig, axes = plt.subplots(1, 2, figsize=(14, 5))
        axes[0].plot(history.history['loss'], label='Training Loss', linewidth=2, color='#FF5722')
        ...
        fig, axes = plt.subplots(1, 2, figsize=(14, 5))
        axes[0].hist(mse, bins=50, color='#FF5722', alpha=0.7, edgecolor='black')
        ...
        axes[0].plot(mse, linewidth=1, color='#FF5722', alpha=0.7)
        ...
        roc_auc = auc(fpr, tpr)
        fig, axes = plt.subplots(1, 2, figsize=(14, 5))
        axes[0].plot(fpr, tpr, color='#FF5722', linewidth=2, label=f'ROC Curve (AUC = {roc_auc:.3f})')
        axes[0].plot([0, 1], [0, 1], color='gray', linestyle='--', linewidth=1, label='Random Classifier')
        axes[0].fill_between(fpr, tpr, alpha=0.3, color='#FF5722')
        ...
        for thresh in threshold_range:
            ...
            tp, fp, fn = np.sum((pred == 1) & (y_true == 1)), np.sum((pred == 1) & (y_true == 0)), np.sum((pred == 0) & (y_true == 1))
        ...
        fig, axes = plt.subplots(1, 2, figsize=(14, 5))
        sns.heatmap(cm, annot=True, fmt='d', cmap='Oranges', ax=axes[0], xticklabels=['Normal', 'Anomaly'], yticklabels=['Normal', 'Anomaly'], annot_kws={'size': 14})
        ...
        width = 0.35
        axes[0, 0].bar(x - width/2, self.processed_data[idx], width, label='Original', color='#FF5722', alpha=0.8)
        axes[0, 0].bar(x + width/2, reconstructions[idx], width, label='Reconstructed', color='#2196F3', alpha=0.8)
        ...
        axes[0, 1].scatter(self.processed_data.flatten(), reconstructions.flatten(), alpha=0.1, s=1, color='#FF5722')
        min_val, max_val = min(self.processed_data.min(), reconstructions.min()), max(self.processed_data.max(), reconstructions.max())
        ...
        axes[0].axis('off')
        layers_info = [('Input\n(4 features)', 1, '#FFF3E0'), (f'Dense\n({self.config["hidden_dim"]} units)', 3, '#FFE0B2'), (f'Dropout\n({self.config["dropout_rate"]})', 4, '#FFCC80'), (f'Bottleneck\n({self.config["bottleneck_dim"]} units)', 5, '#FFB74D'), (f'Dense\n({self.config["hidden_dim"]} units)', 6, '#FFE0B2'), (f'Dropout\n({self.config["dropout_rate"]})', 7, '#FFCC80'), ('Output\n(4 features)', 9, '#FFF3E0')]
...
╚══════════════════════════════════════════╝
"""
        axes[1].text(0.1, 0.5, metrics_text, fontsize=11, family='monospace', verticalalignment='center', transform=axes[1].transAxes, bbox=dict(boxstyle='round', facecolor='#fff3e0', edgecolor='gray'))
        axes[1].set_title('Model Summary', fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'model_summary.png'), dpi=150)
        plt.close()
        return ['training_history.png', 'error_distribution.png', 'error_timeline.png', 'roc_curve.png', 'confusion_matrix.png', 'reconstruction_comparison.png', 'feature_analysis.png', 'model_summary.png']
...
def check_learn_condition():
    try:
        url = f"https://firestore.googleapis.com/v1/projects/{FIREBASE_PROJECT_ID}/databases/(default)/documents/temp/data?key={FIREBASE_API_KEY}"
...
def get_learn_phase_info():
    try:
        ...
        data = response.json()
        fields = data.get('fields', {})
        temp_name = fields.get('temp_name', {}).get('stringValue', 'temp_data')
        phase_name = fields.get('phase_name', {}).get('stringValue', 'phase_1')
        return f"{temp_name}_{phase_name}"
    except:
        return "tempSensorDataFeatures"


def get_user_config_from_firestore():
    try:
        url = f"https://firestore.googleapis.com/v1/projects/{FIREBASE_PROJECT_ID}/databases/(default)/documents/temp/config?key={FIREBASE_API_KEY}"
        response = requests.get(url, timeout=10)
...
def run_training(custom_config=None):
    global status_info
    status_info["is_training"] = True
    status_info["message"] = "Training in progress..."
    temp_dir = tempfile.gettempdir()
    viz_dir = os.path.join(temp_dir, 'temp_visualizations')
    try:
        ...
        detector = TemperatureAnomalyDetector(config=final_config)
        ...
        tflite_path = os.path.join(temp_dir, 'temp_anomaly_detector.tflite')
        params_path = os.path.join(temp_dir, 'temp_params.json')
        detector.convert_to_tflite(tflite_path)
        detector.save_params(params_path)
        
        status_info["message"] = "Uploading to Supabase..."
        upload_to_supabase(tflite_path, 'temp_anomaly_detector.tflite')
        upload_to_supabase(params_path, 'temp_params.json')
        ...
        status_info["last_trained"] = time.strftime("%Y-%m-%d %H:%M:%S")
            ...
            if learn:
                ...
                if countdown_seconds > 0:
                    for remaining in range(countdown_seconds, 0, -1):
                        ...
                        status_info["message"] = f"⏱️ Training starts in {remaining // 60}m {remaining % 60}s..."
                        time.sleep(1)
                        if remaining % 10 == 0:
...
def get_status():
    countdown_text = f"\n\n## ⏱️ COUNTDOWN: {status_info['countdown'] // 60}m {status_info['countdown'] % 60}s" if status_info["countdown"] > 0 else ""
    metrics_text = ""
    if status_info["metrics"]:
        m = status_info["metrics"]
        metrics_text = f"\n## 📊 Last Training Metrics\n| Metric | Value |\n|--------|-------|\n| Reconstruction Accuracy | {m['reconstruction_accuracy']}% |\n| Normal Detection Rate | {m['normal_detection_rate']}% |\n| Final Training Loss | {m['final_training_loss']} |\n| Final Validation Loss | {m['final_validation_loss']} |\n| Threshold | {m['threshold']} |\n| Epochs Trained | {m['epochs_trained']} |\n| Total Samples | {m['total_samples']} |"
    return f"# 📊 Status\n\n**{status_info['message']}**{countdown_text}\n\n---\n\n| Info | Value |\n|------|-------|\n| Is Training | {status_info['is_training']} |\n| Last Trained | {status_info['last_trained'] or 'Never'} |\n| Last Check | {status_info['last_check'] or 'N/A'} |\n{metrics_text}"


def manual_train_with_config(epochs, batch_size, threshold_pct, bottleneck, hidden, dropout, lr, augment, early_stop, patience):
    if status_info["is_training"]:
        return "⚠️ Training already in progress!"
    custom_config = {'epochs': int(epochs), 'batch_size': int(batch_size), 'threshold_percentile': int(threshold_pct), 'bottleneck_dim': int(bottleneck), 'hidden_dim': int(hidden), 'dropout_rate': float(dropout), 'learning_rate': float(lr), 'use_data_augmentation': augment, 'use_early_stopping': early_stop, 'early_stopping_patience': int(patience)}
    threading.Thread(target=run_training, args=(custom_config,)).start()
    return "✅ Training started! Click Refresh to see progress."


with gr.Blocks(title="Temperature Model Trainer") as demo:
    gr.Markdown("# 🌡️ Temperature Anomaly Model Trainer")
    with gr.Tab("📊 Status"):
        status_display = gr.Markdown(get_status())
        gr.Button("🔄 Refresh Status").click(fn=get_status, outputs=status_display)
    with gr.Tab("⚙️ Manual Training"):
        with gr.Row():
        ...
        with gr.Row():
            ...
            bottleneck_input = gr.Slider(1, 3, value=2, step=1, label="Bottleneck Dim")
        with gr.Row():
            hidden_input = gr.Slider(2, 6, value=3, step=1, label="Hidden Dim")
            dropout_input = gr.Slider(0, 0.3, value=0.15, step=0.05, label="Dropout")
        with gr.Row():
        ...
        with gr.Row():
            ...
            patience_input = gr.Slider(5, 30, value=10, step=5, label="Patience")
        result_box = gr.Textbox(label="Result")
        gr.Button("🚀 Start Training", variant="primary").click(fn=manual_train_with_config, inputs=[epochs_input, batch_size_input, threshold_input, bottleneck_input, hidden_input, dropout_input, lr_input, augment_input, early_stop_input, patience_input], outputs=result_box)
