...
class TemperatureAnomalyDetector:
    ...
    def load_data_from_firebase(self, data_path):
        ...
        try:
            ...
            for key, entry in data.items():
                try:
                    feature_vector = [
                        float(entry['tempCurrent']),
                        float(entry['tempDerivative']),
                        float(entry['tempMovingAverage']),
                        float(entry['tempStdDev'])
                    ]
    ...
    def generate_visualizations(self, history, output_dir):
        ...
        layers_info = [
            ('Input\n(4 features)', 1, '#FFF3E0'),
            (f'Dense\n({self.config["hidden_dim"]} units)', 3, '#FFE0B2'),
            (f'Dropout\n({self.config["dropout_rate"]})', 4, '#FFCC80'),
            (f'Bottleneck\n({self.config["bottleneck_dim"]} units)', 5, '#FFB74D'),
            (f'Dense\n({self.config["hidden_dim"]} units)', 6, '#FFE0B2'),
            (f'Dropout\n({self.config["dropout_rate"]})', 7, '#FFCC80'),
            ('Output\n(4 features)', 9, '#FFF3E0')
        ]
...
"""
        axes[1].text(0.1, 0.5, metrics_text, fontsize=11, family='monospace', verticalalignment='center',
                    transform=axes[1].transAxes, bbox=dict(boxstyle='round', facecolor='#fff3e0', edgecolor='gray'))
    ...
    try:
        ...
        if custom_phase and custom_phase.strip():
            try:
                ...
                fields = response.json().get('fields', {})
                temp_name = fields.get('temp_name', {}).get('stringValue', 'temp_data')
                data_path = f"{temp_name}_{custom_phase.strip()}"
            except:
                data_path = f"temp_data_{custom_phase.strip()}"
        else:
        ...
        tflite_path = os.path.join(temp_dir, 'temperature_anomaly_detector.tflite')
        params_path = os.path.join(temp_dir, 'temperature_params.json')
        ...
        upload_to_supabase(tflite_path, 'temperature_anomaly_detector.tflite')
        upload_to_supabase(params_path, 'temperature_params.json')
...
def get_last_training_info_from_supabase():
    ...
    try:
        url = f"{SUPABASE_URL}/storage/v1/object/public/{SUPABASE_BUCKET}/temperature_params.json"
        response = requests.get(url, timeout=10)
        if response.status_code == 200:
...
def get_status_html():
    ...
    <div style="padding:15px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:15px;">
            <div style="background:#fff3e0;padding:10px;border-radius:5px;text-align:center;border:1px solid #FF9800;">
                <div style="font-size:12px;color:#e65100;font-weight:bold;">Last Trained</div>
                <div style="font-size:14px;font-weight:bold;color:#bf360c;">{last_trained or "Never"}</div>
            </div>
            <div style="background:#fce4ec;padding:10px;border-radius:5px;text-align:center;border:1px solid #E91E63;">
                <div style="font-size:12px;color:#c2185b;font-weight:bold;">Data Path</div>
                <div style="font-size:14px;font-weight:bold;color:#880e4f;">{data_path}</div>
            </div>
        </div>
    </div>
    """
...
# Build the interface
with gr.Blocks(title="Temperature Anomaly Model Trainer", theme=gr.themes.Soft()) as demo:
    gr.Markdown("# Temperature Anomaly Detection Model Trainer")
    gr.Markdown("Autoencoder-based anomaly detection for temperature sensor data")
        ...
        with gr.Column(scale=1):
            ...
            with gr.Group():
                ...
                with gr.Row():
                    ...
                    dropout_input = gr.Slider(0, 0.3, value=0.15, step=0.05, label="Dropout Rate")
                with gr.Row():
