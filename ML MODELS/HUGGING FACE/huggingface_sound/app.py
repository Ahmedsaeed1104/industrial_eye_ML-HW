...
class AudioAnomalyDetector:
    ...
    def generate_visualizations(self, history, output_dir):
        ...
        fig, axes = plt.subplots(1, 2, figsize=(14, 5))
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', ax=axes[0], xticklabels=['Normal', 'Anomaly'], yticklabels=['Normal', 'Anomaly'], annot_kws={'size': 14})
...
╚══════════════════════════════════════════╝
"""
        axes[1].text(0.1, 0.5, metrics_text, fontsize=11, family='monospace', verticalalignment='center', transform=axes[1].transAxes, bbox=dict(boxstyle='round', facecolor='#f5f5f5', edgecolor='gray'))
...
with gr.Blocks(title="Sound Model Trainer") as demo:
    gr.Markdown("# 🔊 Sound Anomaly Model Trainer")
    with gr.Tab("📊 Status"):
