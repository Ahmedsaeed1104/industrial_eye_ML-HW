...
def get_data_path_from_firestore():
    try:
        ...
        data_path = f"{sound_name}_{phase_name}"
...
class AudioAnomalyDetector:
    ...
    def convert_to_tflite(self, output_path='sound_anomaly_detector.tflite'):
    ...
    def save_params(self, output_path='sound_params.json'):
    ...
    def generate_visualizations(self, history, output_dir='sound_visualizations'):
...
def get_config_from_firestore():
    try:
        ...
        if 'bottleneck_dim' in fields:
            config['bottleneck_dim'] = int(fields['bottleneck_dim'].get('integerValue', 3))
        if 'hidden_dim' in fields:
            config['hidden_dim'] = int(fields['hidden_dim'].get('integerValue', 6))
...
if __name__ == "__main__":
    print("="*60)
    print("SOUND ANOMALY DETECTION MODEL - KAGGLE TRAINING")
    print("="*60)
    ...
    if len(training_data) == 0:
        ...
        if history:
            ...
            print("\nSaving model files...")
            detector.convert_to_tflite('sound_anomaly_detector.tflite')
            detector.save_params('sound_params.json')
            
            print("\nUploading to Supabase...")
            upload_to_supabase('sound_anomaly_detector.tflite', 'sound_anomaly_detector.tflite')
            upload_to_supabase('sound_params.json', 'sound_params.json')
            
            viz_dir = 'sound_visualizations'
