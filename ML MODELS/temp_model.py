...
def get_data_path_from_firestore():
    try:
        ...
        data_path = f"{temp_name}_{phase_name}"
...
class TemperatureAnomalyDetector:
    ...
    def convert_to_tflite(self, output_path='temp_anomaly_detector.tflite'):
    ...
    def save_params(self, output_path='temp_params.json'):
    ...
    def generate_visualizations(self, history, output_dir='temp_visualizations'):
...
def get_config_from_firestore():
    try:
        ...
        if 'hidden_dim' in fields:
            config['hidden_dim'] = int(fields['hidden_dim'].get('integerValue', 3))
        if 'dropout_rate' in fields:
            config['dropout_rate'] = float(fields['dropout_rate'].get('doubleValue', 0.15))
...
if __name__ == "__main__":
    print("="*60)
    print("TEMPERATURE ANOMALY DETECTION MODEL - KAGGLE TRAINING")
    print("="*60)
    ...
    if len(training_data) == 0:
        ...
        if history:
            ...
            print("\nSaving model files...")
            detector.convert_to_tflite('temp_anomaly_detector.tflite')
            detector.save_params('temp_params.json')
            
            print("\nUploading to Supabase...")
            upload_to_supabase('temp_anomaly_detector.tflite', 'temp_anomaly_detector.tflite')
            upload_to_supabase('temp_params.json', 'temp_params.json')
            
            viz_dir = 'temp_visualizations'
